...
resource "aws_s3_bucket_public_access_block" "nextjs_bucket" {
...
resource "aws_s3_bucket_ownership_controls" "nextjs_bucket" {
    bucket = aws_s3_bucket.nextjs_bucket.id
    rule {
...
resource "aws_s3_bucket_acl" "nextjs_bucket" {
    depends_on = [
        aws_s3_bucket_ownership_controls.nextjs_bucket,
        aws_s3_bucket_public_access_block.nextjs_bucket,
    ]
...
data "aws_iam_policy_document" "nextjs_public_access" {
    statement {
        principals {
            type  = "*"
            identifiers = ["*"]
...
resource "aws_s3_bucket_policy" "nextjs_public_access" {
    bucket = aws_s3_bucket.nextjs_bucket.id
    policy = data.aws_iam_policy_document.nextjs_public_access.json
	
	depends_on = [aws_s3_bucket_acl.nextjs_bucket]
}
...
resource "aws_cloudfront_distribution" "s3_distribution" {
	...
	default_root_object = "index.html"
  	...
  	# aliases = ["url-shortener-fe.co.uk"]
